...
This overlay inherits from the Gentoo repository through `masters = gentoo` in `metadata/layout.conf`, so your Portage configuration must expose the main Gentoo repository with the repo name `gentoo`. That requirement is about the Portage repo name itself, whether your system manages it through `eselect repository` or through the default `::gentoo` configuration.
